get_oc_Ivanova_continuous <- function(target, eps = 1, ptox,ncohort,cohortsize, n.earlystop = 100, ntrial,startdose=1, seed = 100){
  if (n.earlystop <= 6) {
  ...
  for(trial in 1:ntrial){
    res = nonpart_continuous(ga=target,ptox,ncohort,cohortsize,startdose, eps = eps, n.earlystop1 = n.earlystop)
    N[trial, ] = res$trials
